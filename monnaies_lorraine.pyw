        ...
        cursor.execute('''
            WITH ordered AS (
                SELECT
                    ...
                    ROW_NUMBER() OVER (
                        ORDER BY localite,
                        CAST(substr(periode_annee, 1, instr(periode_annee || '-', '-') - 1) AS INT),
...
class MonnaiesApp:
    def __init__(self, root):
        ...
        ttk.Button(self.button_frame, text="Dupliquer une fiche", command=self.duplicate_fiche).pack(side=tk.LEFT, padx=5)
        ...
        self.count_label = self.number_of_results = ttk.Label(self.button_frame, text="Nombre de fiches :")
    ...
    def load_data(self):
        try:
            ...
            self.count_label.config(text=f"Nombre de fiches : {len(rows)}")
        except sqlite3.Error as e:
    ...
    def duplicate_fiche(self):
        ...
        selected_item = selected_items[0]
        fiche_id = self.tree_liste.item(selected_item)["values"][0]
        formulaire_fiche = FicheDetailWindow(self, None, insert_after_id=fiche_id)
        conn = sqlite3.connect("data/monnaies.db")
        cursor = conn.cursor()
        cursor.execute("SELECT attribution, localite, atelier, metal FROM monnaies WHERE id = ?", (fiche_id,))
        result = cursor.fetchone()
        formulaire_fiche.attribution_combobox.set(result[0])
        formulaire_fiche.localite_combobox.set(result[1])
        formulaire_fiche.atelier_combobox.set(result[2])
        formulaire_fiche.metal_combobox.set(result[3])
        cursor.close()
        conn.close()
